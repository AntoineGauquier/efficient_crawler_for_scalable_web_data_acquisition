...
class DFSOnlineCrawler(scrapy.Spider):
    name = "dfs_online_crawler"
    handle_httpstatus_list = [i for i in range(300, 600)]
    
    @classmethod
    def from_crawler(cls, crawler, *args, **kwargs):

        spider = super(DFSOnlineCrawler, cls).from_crawler(crawler, *args, **kwargs)
 
        crawler.settings.set('LOG_FILE', os.path.join("crawlers", "dfs_online_crawler", "logs", "logs_" + kwargs.get('site_name', None) + "_" + datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + ".log"))

        if not os.path.exists(os.path.join("crawlers", "dfs_online_crawler", "logs")):
            os.mkdir(os.path.join("crawlers", "dfs_online_crawler", "logs"))
        if not os.path.exists(os.path.join("crawlers", "dfs_online_crawler", "output")):
            os.mkdir(os.path.join("crawlers", "dfs_online_crawler", "output"))
