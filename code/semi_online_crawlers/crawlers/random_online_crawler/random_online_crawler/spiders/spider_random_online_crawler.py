...
class RandomOnlineCrawler(scrapy.Spider):
    name = "random_online_crawler"
    handle_httpstatus_list = [i for i in range(300, 600)]
    
    @classmethod
    def from_crawler(cls, crawler, *args, **kwargs):

        spider = super(RandomOnlineCrawler, cls).from_crawler(crawler, *args, **kwargs)
 
        crawler.settings.set('LOG_FILE', os.path.join("crawlers", "random_online_crawler", "logs", "logs_" + kwargs.get('site_name', None) + "_" + datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + ".log"))

        if not os.path.exists(os.path.join("crawlers", "random_online_crawler", "logs")):
            os.mkdir(os.path.join("crawlers", "random_online_crawler", "logs"))
        if not os.path.exists(os.path.join("crawlers", "random_online_crawler", "output")):
            os.mkdir(os.path.join("crawlers", "random_online_crawler", "output"))
    ...
    def is_crawling_terminated(self):
        return not self.frontier_list

    def get_next_link(self):
        next_link = random.choice(self.frontier_list)
        idx_next_link = self.frontier_dict[next_link]

        last = self.frontier_list[-1]
        self.frontier_list[idx_next_link] = last
        self.frontier_dict[last] = idx_next_link

        self.frontier_list.pop()
        del self.frontier_dict[next_link]

        return next_link

    def add_link(self, link):
        self.frontier_list.append(link)
        self.frontier_dict[link] = len(self.frontier_list) - 1

    def is_link_in_frontier(self, link):
        return link in self.frontier_dict
