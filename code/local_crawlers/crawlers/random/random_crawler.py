...
class RandomCrawler(GenericLocalCrawler):
    ...
    def get_next_link(self):
        chosen_link = choice(self.frontier)
        self.frontier.remove(chosen_link)
        return chosen_link
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        ...
        if not os.path.exists(os.path.join(os.getcwd(), "crawlers/random/logs")):
            os.mkdir(os.path.join(os.getcwd(), "crawlers/random/logs"))
        if not os.path.exists(os.path.join(os.getcwd(), "crawlers/random/logs", str(log_path_all_sites))):
            os.mkdir(os.path.join(os.getcwd(), "crawlers/random/logs", str(log_path_all_sites)))
        ...
        bfs_crawler = RandomCrawler(db_path=db_path, 
            table_name=table_name, 
            log_path=log_path, 
            starting_url=starting_url, 
            use_url_classifier=use_url_classifier, 
