...
case $method in
    ...
    2)
		...
		python3 local_crawlers/offline_dom/offline_dom_crawler.py "$threshold" "$m" "$w" "$budget" "$n" 0 "$log_path_all_sites" "$site_name"
	;;
	3)
		clear
		...
		python3 local_crawlers/bfs/bfs.py "$budget" "$log_path_all_sites" "$site_name"
	;;
	4)
		clear
		...
		python3 local_crawlers/dfs/dfs.py "$budget" "$log_path_all_sites" "$site_name"
	;;
	5)
		...
		python3 local_crawlers/random/random_crawler.py "$budget" "$log_path_all_sites" "$site_name"
	;;
esac
