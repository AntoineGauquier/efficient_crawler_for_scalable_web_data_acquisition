...
def plot_results(name_of_plot, locations, site_name, legend, colors, markers):
    ...
    for dir_r in os.listdir(os.path.join(os.getcwd(), 'local_crawlers', locations[-1], site_name)):
        max_len_site_nb = len(list(np.load(os.path.join(os.getcwd(), 'local_crawlers', locations[-1], site_name, dir_r, "nb_data_resources.npy"))))
        data_volume_r = np.load(os.path.join(os.getcwd(), 'local_crawlers', locations[-1], site_name, dir_r, "data_volumes.npy"), allow_pickle = True)
    ...
    for method_idx, method in enumerate(locations):
        ...
        site_path = os.path.join(os.getcwd(), 'local_crawlers', method, site_name)
